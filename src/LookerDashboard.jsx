...
function monthlyFrom(rows) {
  ...
  rows.forEach((row) => {
    ...
    const m = (map[key] ||= { key, y: row.date.getFullYear(), mo: row.date.getMonth(),
...
// Parse a date that may arrive as YYYY-MM-DD, DD/MM/YYYY, or a Date.
function parseDate(v) {
  if (v instanceof Date) return v;
  const s = String(v || "").trim();
  // YYYY-MM-DD (from backend toISO)
  let m = s.match(/^(\d{4})-(\d{1,2})-(\d{1,2})$/);
  if (m) return new Date(+m[1], +m[2] - 1, +m[3]);
  // DD/MM/YYYY (raw sheet format)
  ...
  return (
    <div style={{ minWidth: 232 }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}>
        <button onClick={() => setView(new Date(y, mo - 1, 1))} style={navBtn(t)}><ChevronLeft size={16} /></button>
        <span style={{ fontSize: 13.5, fontWeight: 600 }}>
          {view.toLocaleString("en-US", { month: "long" })} {y}
        </span>
        <button onClick={() => setView(new Date(y, mo + 1, 1))} style={navBtn(t)}><ChevronRight size={16} /></button>
      </div>
      <div style={{ display: "grid", gridTemplateColumns: "repeat(7,1fr)", gap: 2 }}>
        {["M","T","W","T","F","S","S"].map((d, i) => (
          <div key={i} style={{ textAlign: "center", fontSize: 10.5, color: t.sub, fontWeight: 600, padding: "2px 0" }}>{d}</div>
        ))}
        {cells.map((d, i) => {
          ...
          return (
            <button key={i} onClick={() => onPick(d)} style={{
              border: "none", cursor: "pointer", padding: "6px 0", fontSize: 12.5, borderRadius: 7,
...
function DateRangePicker({ t, range, setRange, min, max }) {
  ...
  const presets = [
    ["This year", () => [new Date(thisYear, 0, 1), max]],
    ["Last 6 months", () => [new Date(max.getFullYear(), max.getMonth() - 5, 1), max]],
    ["Last 30 days", () => [new Date(max.getTime() - 30 * DAY), max]],
    ["Q1 " + thisYear, () => [new Date(thisYear, 0, 1), new Date(thisYear, 2, 31)]],
    ["FY " + (thisYear - 1) + "-" + String(thisYear).slice(2), () => [new Date(thisYear - 1, 3, 1), new Date(thisYear, 2, 31)]],
    ["All time", () => [min, max]],
  ];
  ...
  return (
    <div ref={ref} style={{ position: "relative" }}>
      ...
      {open && (
        <div style={{ position: "absolute", top: "calc(100% + 8px)", right: 0, zIndex: 70,
          background: t.card, border: `1px solid ${t.border}`, borderRadius: 14,
          boxShadow: "0 12px 40px rgba(60,64,67,.25)", padding: 18, animation: "rise .15s both",
            ...
            {presets.map(([n, fn]) => (
              <button key={n} onClick={() => { const [s, e] = fn(); setStart(s); setEnd(e); setVStart(s); setVEnd(e); }}
                style={{ textAlign: "left", border: "none", background: "transparent", color: t.text,
                  fontSize: 12.5, padding: "7px 10px", borderRadius: 7, cursor: "pointer" }}
                onMouseEnter={(e) => e.currentTarget.style.background = t.hover}
          ...
          <div>
            <div style={{ display: "flex", gap: 22, flexWrap: "wrap" }}>
              <div>
                <div style={{ textAlign: "center", fontSize: 11.5, fontWeight: 600, color: t.sub, marginBottom: 8 }}>Start date</div>
                <Calendar t={t} view={vStart} setView={setVStart} value={start}
                  rangeStart={start} rangeEnd={end}
                  onPick={(d) => { setStart(d); if (end && d > end) setEnd(d); }} />
              </div>
              <div>
                <div style={{ textAlign: "center", fontSize: 11.5, fontWeight: 600, color: t.sub, marginBottom: 8 }}>End date</div>
...
/* ----------------------------------------------------------------
   AUTH — mirrors Google Sheet tab "pass"
   Columns: A UserID | B FullName | C Email | D Mobile | E Username
            F Password | G Role | H Status | I CreatedDate | J LastLogin | K PhotoURL
----------------------------------------------------------------- */
        ...
        {mode === "login" ? (
          <>
            ...
            <button onClick={async () => {
              ...
              setBusy(false);
              if (r.ok) onLogin(r.user);
              else setErr("Google login requires Firebase setup. Use username/password.");
...
export default function App() {
  ...
  // Fetch data from Google Sheet when user logs in
  useEffect(() => {
    if (!user) return;
    setLoading(true);
    api("getData").then((r) => {
  ...
  // Auto-refresh every 5 minutes (fresh=true bypasses server cache)
  useEffect(() => {
    if (!user) return;
    const id = setInterval(() => {
      api("getData", { fresh: true }).then((r) => {
        if (r.ok && r.rows) setRows(r.rows.map(normalizeRow).filter((x) => x.name));
      });
    }, 5 * 60 * 1000);
  ...
  const dataDateRange = useMemo(() => {
    const fallback = { min: new Date(2024, 3, 1), max: new Date(2026, 2, 31) };
    if (!rows.length) return fallback;
    const dates = rows.map((r) => r.date).filter((d) => d instanceof Date && !isNaN(d) && d.getFullYear() > 2000 && d.getFullYear() < 2040);
  ...
  const removeUser = async (u) => {
    if (!window.confirm(`Delete user "${u.username}"? This removes the row from the pass sheet.`)) return;
    setAdminBusy(true);
  ...
  const monthEmployees = useMemo(() => {
    ...
    return employeesFrom(filtered.filter((r) => `${r.date.getFullYear()}-${r.date.getMonth()}` === activeMonthKey));
  }, [filtered, activeMonthKey]);

  const yearEmployees = useMemo(() => {
    if (!activeYear) return [];
    return employeesFrom(filtered.filter((r) => String(r.date.getFullYear()) === String(activeYear)));
  }, [filtered, activeYear]);
  ...
  const notifications = useMemo(() => {
    ...
    list.push({ icon: Activity, color: t.primary,
      text: `${fmt(rows.length)} records loaded from Google Sheets`, time: "just now" });
    if (kpis.pending > 0)
    ...
    <div style={{ minHeight: "100vh", display: "grid", placeItems: "center",
      ...
      <div style={{ textAlign: "center" }}>
        <div style={{ width: 40, height: 40, border: `3px solid ${t.border}`,
          ...
          margin: "0 auto 16px" }} />
        <div style={{ color: t.text, fontWeight: 600, fontSize: 16 }}>Loading data from Google Sheets…</div>
  ...
  const empDetailTable = (list) => (
    <div className="lk-scroll" style={{ overflow: "auto", maxHeight: 520 }}>
  ...
  return (
    <div style={{ display: "flex", minHeight: "100vh", width: "100%", maxWidth: "100vw", overflowX: "hidden",
      ...
      <aside className={`lk-sidebar ${sidebar ? "open" : ""}`} style={{
        width: 240, flexShrink: 0, background: t.card, borderRight: `1px solid ${t.border}`,
        ...
        {NAV.map((n) => {
          const on = tab === n.id;
          return (
            <button key={n.id} onClick={() => { setTab(n.id); setSidebar(false); setNotifOpen(false); }} style={{
      ...
      <div style={{ flex: 1, display: "flex", flexDirection: "column", minWidth: 0 }}>
        ...
        <header className="lk-header" style={{ display: "flex", alignItems: "center", gap: 14, padding: "13px 24px",
          background: dark ? "rgba(38,40,44,.85)" : "rgba(255,255,255,.85)", backdropFilter: "blur(12px)",
          ...
          <div className="lk-search" style={{ display: "flex", alignItems: "center", gap: 9, background: t.hover,
            ...
            <input value={search} onChange={(e) => setSearch(e.target.value)}
              placeholder="Search employees or departments…" style={{ border: "none", outline: "none",
                background: "transparent", color: t.text, fontSize: 13.5, width: "100%", boxShadow: "none" }} />
            {search && <X size={15} color={t.sub} style={{ cursor: "pointer" }} onClick={() => setSearch("")} />}
          </div>
        ...
        <div className="lk-titlebar" style={{ display: "flex", flexWrap: "wrap", gap: 14, padding: "22px 24px 4px",
        ...
        <main className="lk-main" style={{ padding: "18px 24px 28px", display: "flex", flexDirection: "column", gap: 20 }}>
          {/* KPI GRID */}
          ...
          {tab === "dept" && (
            <>
              <Panel t={t} title="Department Performance Ranking" style={{ padding: 0 }}>
                <div className="lk-scroll" style={{ overflow: "auto", maxHeight: 440 }}>
                  <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
                    <thead><tr>
                      ...
                      {deptAgg.map((d, i) => (
                        <tr key={d.dept} style={{ borderTop: `1px solid ${t.border}` }}>
                          ...
                          <td style={{ padding: "11px 12px", textAlign: "right",
                            background: pendingBg(d.pending, Math.max(...deptAgg.map(x=>x.pending))) }}>{fmt(d.pending)}</td>
          ...
          {tab === "emp" && (
            <Panel t={t} title={`Employee Performance · ${sorted.length} employees`} style={{ padding: 0 }}>
              <div ref={scrollRef} onScroll={onTableScroll} className="lk-scroll" style={{ overflow: "auto", maxHeight: 560 }}>
                <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
                  <thead><tr>
          ...
          {tab === "monthly" && (
            ...
            <Panel t={t} style={{ padding: 0 }}
              ...
              <div className="lk-scroll" style={{ overflow: "auto", maxHeight: 480 }}>
                <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
                  <thead><tr>
                  ...
                  <tbody>
                    ...
                    {scoringData.map((g, i) => (
                      <tr key={g.label} className="lk-row" style={{ borderTop: `1px solid ${t.border}` }}
                        onMouseEnter={(e)=>e.currentTarget.style.background=t.hover}
                        onMouseLeave={(e)=>e.currentTarget.style.background="transparent"}>
                        <td style={{ padding: "11px 12px", color: t.sub, fontWeight: 600 }}>{i + 1}</td>
            ...
            <Panel t={t} title={`Monthly Breakdown · ${MONTHLY.length} months`} style={{ padding: 0 }}>
              <div className="lk-scroll" style={{ overflow: "auto", maxHeight: 360 }}>
                <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
                  <thead><tr>
          ...
          {tab === "scores" && (
            <Panel t={t} title={`Score Summary · ${scoreSummary.length} employees`} style={{ padding: 0 }}>
              <div className="lk-scroll" style={{ overflow: "auto", maxHeight: 620 }}>
                <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
                  <thead><tr>
                  ...
                  <tbody>
                    ...
                    {scoreSummary.map((r, i) => (
                      <tr key={r.name} className="lk-row" style={{ borderTop: `1px solid ${t.border}` }}
                        onMouseEnter={(e)=>e.currentTarget.style.background=t.hover}
                        onMouseLeave={(e)=>e.currentTarget.style.background="transparent"}>
                        <td style={{ padding: "11px 12px", color: t.sub, fontWeight: 600 }}>{i + 1}</td>
                        <td style={{ padding: "11px 12px", fontWeight: 600 }}>{r.name}</td>
          ...
          {tab === "admin" && isAdmin && (
            ...
            <Panel t={t} style={{ padding: 0 }}
              ...
              <div className="lk-scroll" style={{ overflow: "auto", maxHeight: 540 }}>
                <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
                  <thead><tr>
              ...
              <div style={{ padding: "10px 18px", borderTop: `1px solid ${t.border}`, fontSize: 12.5, color: t.sub }}>
                Blocking a user prevents login · Deleting removes them from the pass sheet · You can't block or delete yourself
              </div>
            </Panel>
